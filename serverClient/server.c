...
{   
    ...
    {
        printf("Socketpair-ul a esuat.. \n");
        write(s_to_c_fifo, "Socketpair-ul din comanda get-logged-users a esuat.. \n", 54);
        return;
    }
    int child = fork();
    ...
    if (child == 0) // ------------ get-logged-users ------- copil
    {
        ...
        // write(s_to_c_fifo, response, strlen(response));

        // strcat(response, "\0");
        // printf("[copil1] %s", response);
        write(sockp[1], response, strlen(response));
        // printf("[copil2] %s", response);
        // memset(response, 0, sizeof(response)); // altfel se pastra data-n functia get-logged-users
    ...
    else // --------- get-logged-users -------- parinte
    {
        ...
        int length = read(sockp[0], s, sizeof(s) - 1);
        if (length < 0)
        {
...
void get_proc_info(int pid, int s_to_c_fifo)
{
    ...
    if (child == 0) // ------------ get-proc-info ------- copil
    {
        ...
        if (s_file == NULL)
        {
            ...
            write(_pipefd[1], "Eroare la deschiderea ' /proc/pid/status'.. \n", 46);
            close(_pipefd[1]);
            exit(1);
        }
        else
        {
            ...
            if (write(_pipefd[1], res, length) < 0)
            {
...
void process_message(char *s, int s_to_c_fifo)
{
    ...
    strcpy(command, s);
    // if (strncmp(command, "", 0) == 0)
    // {
    //     printf("Comanda vida.. \n");
    //     write(s_to_c_fifo, "Comanda vida.. \n", 16);
    //     return;
    // }
